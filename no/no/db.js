...
class Database {
  constructor(
    ...
    if (this.HOSTNAME.toLowerCase().startsWith("http")) {
      throw new Error(
        'Error DB2: Hostname *MUST NOT* start with "http" or "https"\n'
      );
    }
    ...
    this._wss.on("connection", (client) => {
      if (!client) {
        throw new Error(
          "Error DB4: Invalid or null client, an internal error occurred"
        );
      }
      ...
      client.on("message", (JSONData) => {
        ...
        if (data.PROTOCOL > 7 || data.PROTOCOL < 0) {
        ...
        if (data.PROTOCOL === PROTOCOL.EXISTS) {
          ...
          const LogDetails = this.CreateUserIfPossible(
            data.USERNAME,
            data.PASSWORD
          );
          client.send(
          ...
          const LogDetails = this.ConnectUserIfPossible(
            data.USERNAME,
            data.PASSWORD,
            data.COOKIE
          );
          ...
          const LogDetails = this.StoreDataIfPossible(
            data.USERNAME,
            data.COOKIE,
            data.DATA
          );
          ...
          const LogDetails = this.DeleteUserIfPossible(
            data.USERNAME,
            data.PASSWORD
          );
          client.send(
          ...
          const LogDetails = this.DeleteDataIfPossible(
            data.USERNAME,
            data.PASSWORD
          );
          client.send(
          ...
          const LogDetails = this.ConnectWithoutLogging(
            data.USERNAME,
            data.COOKIE
          );
          client.send(
  ...
  UserExists(USERNAME = "") {
    if (!this.__DATA__.USERS) {
      throw new Error(
        'Error DB6: Database does not contain any property "USERS", The data might be currupted'
      );
    }
    let exists = false;
  ...
  ConnectWithoutLogging(USERNAME, COOKIE) {
    ...
    if (d) {
      ...
      return {
        RESULT: false,
        REASON: "Invalid User ID, Please try re-logging into your ACCOUNT",
      };
    }
  }
  ...
  DeleteDataIfPossible(USERNAME, PASSWORD) {
    ...
    for (let i of this.__DATA__.USERS) {
      if (USERNAME === i.USERNAME && PASSWORD === i.PASSWORD) {
        ...
        return {
          RESULT: true,
          REASON: "Successfully deleted your ACCOUNT'S DATA",
        };
      }
      index++;
    }
      ...
      if (USERNAME === i.USERNAME) {
        if (i.COOKIES.includes(COOKIE)) {
          ...
          return {
            RESULT: false,
            REASON:
              "Invalid User ID, Please try to re-logging into your ACCOUNT",
          };
        }
      }
      index++;
  ...
  Save() {
    fs.writeFile(
      "database.json",
      JSON.stringify(this.__DATA__, undefined, 4),
      (err) => {
        if (err) console.log("Error Saving the data:\n" + err);
      }
    );
  }
}
